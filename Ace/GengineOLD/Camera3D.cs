...
namespace Ace.Gengine
{
	  /// <summary>
	  /// This is a camera i basically remade to make it work. Using quite a bit of stuff from my
	  /// camera class its nearly the same as mine but its a bit simpler. I have bunches of
	  /// cameras at this point and i need to combine them into a fully hard core non basic
	  /// camera. That said simple makes for a better example and a better basis to combine them
	  /// later.
	  /// </summary>
	  public class Camera3D
	  {
		    ...
		    /// <summary> operates pretty much like a fps camera. </summary>
		    public const int CAM_UI_OPTION_FPS_LAYOUT = 0;

		    /// <summary>
		    /// I put this one on by default. free cam i use this for editing its more like a
		    /// air plane or space camera. the horizon is not corrected for in this one so use
		    /// the z and c keys to roll hold the right mouse to look with it.
		    /// </summary>
		    public const int CAM_UI_OPTION_EDIT_LAYOUT = 1;

		    /// <summary> Determines how the camera behaves fixed 0 free 1 </summary>

		    /// <summary>
		    /// A fixed camera is typically used in fps games. It is called a fixed camera
		    /// because the up is stabalized to the system vectors up. However be aware that
		    /// this means if the forward vector or were you are looking is directly up or down
		    /// you will gimble lock. Typically this is not allowed in many fps or rather it is
		    /// faked so you can never truely look directly up or down.
		    /// </summary>
		    public const int CAM_TYPE_OPTION_FIXED = 0;

		    /// <summary>
		    /// A free camera has its up vector unlocked good for a space sim, air fighter game
		    /// or editing. It won't gimble lock. Provided the up is found from the cross of the
		    /// right forward it can't gimble lock. The draw back is the horizon stabilization
		    /// needs to be handled for some types of games.
		    /// </summary>
		    ...
		    /// <summary>
		    /// Select how you want the ui to feel or how to control the camera by passing in
		    /// Basic3dExampleCamera. CAM_UI_ options
		    /// </summary>
		    ...
		    /// <summary>
		    /// Select a camera type fixed free or other by passing in ( Basic3dExampleCamera.
		    /// CAM_TYPE_ options )
		    /// </summary>
		    ...
		    /// <summary>
		    /// This serves as the cameras up. For fixed cameras this might not change at all
		    /// ever. For free cameras it changes constantly. A fixed camera keeps a fixed
		    /// horizon but can gimble lock under normal rotation when looking straight up or
		    /// down. A free camera has no fixed horizon but can't gimble lock under normal
		    /// rotation as the up changes as the camera moves. Most hybrid cameras are a blend
		    /// of the two but all are based on one or both of the above.
		    /// </summary>
		    private Vector3 up = Vector3.Up;

		    /// <summary>
		    /// This serves as the cameras world orientation like almost all 3d game objects
		    /// they have a world matrix. It holds all orientational values and is used to move
		    /// the camera properly thru the world space.
		    /// </summary>
		    private Matrix camerasWorld = Matrix.Identity;

		    /// <summary>
		    /// The view matrice is created from the cameras world matrice but it has special
		    /// properties. Using create look at to create this matrix you move from the world
		    /// space into the view space. If you are working on world objects you should not
		    /// take individual elements from this to directly operate on world matrix
		    /// components. As well the multiplication of a view matrix by a world matrix moves
		    /// the resulting matrix into view space itself.
		    /// </summary>
		    private Matrix viewMatrix = Matrix.Identity;

		    /// <summary>
		    /// The projection matrix. This matrice creates a vanishing point and skews all
		    /// objects drawn to create the illusion of depth and a perspective parallax view at
		    /// distance.
		    /// </summary>
		    private Matrix projectionMatrix = Matrix.Identity;

		    /// <summary> Gets or sets the the camera's position in the world. </summary>
		    public Vector3 Position
		    {
				set
				{
					  camerasWorld.Translation = value;
					  // since we know here that a change has occured to the cameras world
					  // orientations we can update the view matrix.
					  ReCreateWorldAndView();
				}
		    ...
		    /// <summary>
		    /// Gets or Sets the direction the camera is looking at in the world. The forward is
		    /// the same as the look at direction it i a directional vector not a position.
		    /// </summary>
		    ...
		    /// <summary>
		    /// Get the cameras up vector. You shouldn't need to set the up you shouldn't at all
		    /// if you are using the free camera type.
		    /// </summary>
		    public Vector3 Up
		    {
				set
		    ...
		    /// <summary>
		    /// Gets or Sets the direction the camera is looking at in the world as a
		    /// directional vector.
		    /// </summary>
		    ...
		    /// <summary> Sets a positional target in the world to look at. </summary>
		    ...
		    /// <summary>
		    /// Turns the camera to face the target this method just takes in the targets matrix
		    /// for convienience.
		    /// </summary>
		    ...
		    /// <summary>
		    /// Gets the view matrix we never really set the view matrix ourselves outside this
		    /// method just get it. The view matrix is remade internally when we know the world
		    /// matrix forward or position is altered.
		    /// </summary>
		    public Matrix View
		    {
				get
		    ...
		    /// <summary>
		    /// When the cameras position or orientation changes, we call this to ensure that
		    /// the cameras world matrix is orthanormal. We also set the up depending on our
		    /// choices of is fix or free camera and we then update the view matrix.
		    /// </summary>
		    ...
		    /// <summary>
		    /// Changes the perspective matrix to a new near far and field of view. The
		    /// projection matrix is typically only set up once at the start of the app.
		    /// </summary>
		    ...
		    /// <summary> update the camera. </summary>
		    ...
		    /// <summary>
		    /// like a fps games camera right clicking turns mouse look on or off same for the
		    /// edit mode.
		    /// </summary>
		    ...
		    /// <summary> when working like programing editing and stuff. </summary>
		    /// <param name="gameTime"> </param>
		    ...
		    /// <summary>
		    /// This function can be used to check if gimble is about to occur in a fixed
		    /// camera. If this value returns 1.0f you are in a state of gimble lock, However
		    /// even as it gets near to 1.0f you are in danger of problems. In this case you
		    /// should interpolate towards a free camera. Or begin to handle it. Earlier then .9
		    /// in some manner you deem to appear fitting otherwise you will get a hard spin
		    /// effect. Though you may want that.
		    /// </summary>
		    ...
		    #endregion Local Translations and Rotations.
		    ...
		    /// <summary>
		    /// These aren't typically useful and you would just use create world for a camera
		    /// snap to a new view. I leave them for completeness.
		    /// </summary>
		    ...
		    #endregion Non Local System Translations and Rotations.
	  }
}